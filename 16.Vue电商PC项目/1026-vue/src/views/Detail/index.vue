...
export default {
  ...
  methods: {
    ...mapActions(["getGoodsDetail", "addCartList"]),
    setChecked(saleAttrValue, saleAttrValueList) {
      // 将其他的改为0
      saleAttrValueList.forEach((value) => {
    ...
    async addCart() {
      try {
        ...
        // 2. 请求成功，存储数据到sessionStorage
        /* eslint-disable-next-line */
        const { id, skuName, skuDesc, skuDefaultImg, price } = this.skuInfo;
